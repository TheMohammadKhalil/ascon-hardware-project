...
module ascon_core (
  ...
  assign abs_msg_part = (fsm_q == ABS_MSG) && (bdi_type == D_MSG) && (bdi_valid != 5'd0) && bdi_ready;
  ...
  assign sqz_hash_done2 = ((hash_cnt_q == 5'd3) && sqz_hash_done1) || (sqz_hash && bdo_eoo);
  assign sqz_tag        = (fsm_q == SQZ_TAG) && bdo_valid && bdo_ready;
  assign sqz_tag_done   = (word_cnt_q == (W128 - 1)) && sqz_tag;
  assign ver_tag        = (fsm_q == VER_TAG) && (bdi_type == D_TAG) && bdi_ready && (bdi_valid != 5'd0);
  ...
  always_comb begin
    ...
    unique case (fsm_q)
      ...
      ABS_MSG: begin
        ...
        bdo_valid = (mode_enc_dec && (bdi_valid != 5'd0)) ? 'd1 : 'd0;
        bdo_type  = mode_enc_dec ? D_MSG : D_INVALID;
        bdo_eot   = mode_enc_dec ? bdi_eot : 'd0;
        if (mode_q == M_HASH256) bdo = 5'd0;
      end
      SQZ_TAG: begin
        state_idx = word_cnt_q + W192;
        bdo       = state_slice;
      ...
      SQZ_HASH: begin
        ...
        bdo_valid = 5'd1;
        bdo_type  = D_HASH;
        bdo_eot   = (hash_cnt_q == 5'd3) && (word_cnt_q == (W64 - 1));
      end
      VER_TAG: begin
        state_idx = word_cnt_q + W192;
        bdi_ready = 5'd1;
  ...
  always_comb begin
    ...
    if (fsm_q == PAD_AD) word_cnt_d = 5'd0;
    if (fsm_q == PAD_MSG) word_cnt_d = 5'd0;
  end
  ...
  always_comb begin
    ...
    if (mode_q == M_HASH256) begin
      ...
      if (abs_ad_done && bdi_eoi) hash_cnt_d = 5'd0;
    end
  end
  ...
  always_comb begin
    ...
    if (ld_npub_done) begin
      if (bdi_eoi) eoi_d = 5'd1;
    end
    if (abs_ad_done) begin
      if (bdi_eot) ad_eot_d = 5'd1;
      if (bdi_eoi) eoi_d    = 5'd1;
    end
    if (add_ad_pad) ad_pad_d = 5'd1;
    if (add_msg_pad) ad_pad_d = 5'd1;
    if (abs_msg_done && bdi_eoi) eoi_d = 5'd1;
    if (kadd_4_done && (mode_q == M_AEAD128_DEC)) auth_intern_d = 5'd1;
